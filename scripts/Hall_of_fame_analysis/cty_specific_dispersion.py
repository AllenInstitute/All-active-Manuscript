...
def draw_heatmap(figname,dist_matrix,patch_length=40):
    ...
    im_ = ax_.imshow(dist_matrix,vmin=0,vmax=0.5,cmap='Blues')
    
    ax_.set_xlabel('Hall of Fame Models',fontsize=axis_fontsize)
    ax_.set_ylabel('Hall of Fame Models',fontsize=axis_fontsize)
    ax_.set_title(bcre_,fontsize=title_fontsize)
    ax_.set_xticklabels(['']*len(ax_.get_xticklabels()))
    ax_.set_yticklabels(['']*len(ax_.get_yticklabels()))
    ax_.grid(False)
    current_idx = 0
    for _ in range(len(unique_cell_ids)):
        ax_.add_patch(patches.Rectangle((current_idx, current_idx),
                                      patch_length, # Width
                                      patch_length, # Height
                                      facecolor="none",
                                      edgecolor='k',
    ...
    cbar_ = plt.colorbar(im_,ax=ax_,cax=cax,ticks=[0,.5])
    cax.tick_params(labelsize=tick_fontsize)
    cbar_.outline.set_visible(False)
    cax.set_ylabel('Normalized distance',fontsize=axis_fontsize)
...
fig,ax = plt.subplots(2,len(bcre_order)//2,sharex=True,sharey=True,figsize=(5,5))
...
#    
    ...
    heatmap_figname = 'figures/intra_inter_distbn_heatmap_%s.png'%bcre_
...
fig.savefig('figures/intra_inter_distbn.pdf',bbox_inches='tight')
plt.close(fig)
