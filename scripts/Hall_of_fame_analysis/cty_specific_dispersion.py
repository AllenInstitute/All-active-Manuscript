...
def draw_heatmap(figname, dist_matrix, patch_length=40):
    ...
    ax_.set_title(subclass_, fontsize=fontsize)
...
def process_conductance_data(hof_param_data, filter_list=['Cell_id', 'hof_index', 'Broad_subclass']):
    hof_param_data_subclass_ = hof_param_data.dropna(axis=1, how='any')
    conductance_params = [dens_ for dens_ in list(hof_param_data_subclass_)
                          if dens_ not in filter_list]
    conductance_val_bcre_ = hof_param_data_subclass_.loc[:,
                                                         conductance_params].values

    conductance_val_bcre_ = scaler.fit_transform(
        conductance_val_bcre_) / conductance_val_bcre_.shape[1]
    conductance_dist_matrix = euclidean_distances(conductance_val_bcre_)
    return hof_param_data_subclass_, conductance_dist_matrix
...
def draw_significance(mu_dist1, mu_dist2, pval, ax, height_offset=.1):
    ...
    ax.plot([mean_dist1, mean_dist1, mean_dist2, mean_dist2], [
            y_height1, max_y_height, max_y_height, y_height2], color='k')
...
hof_param_subclass = pd.merge(
    hof_param_data, subclass_cluster, how='left', on='Cell_id')
broad_subclass_color_dict = utility.load_pickle(broad_subclass_colors_filename)

broad_subclass = [subclass for subclass in broad_subclass_color_dict.keys()
                  if subclass != 'Other']
...
fig, ax = plt.subplots(2, len(broad_subclass) // 2, sharex=False,
                       sharey=False, figsize=(5, 5))

for ii, subclass_ in enumerate(broad_subclass):
    hof_param_data_subclass_ = (
        hof_param_subclass.loc[hof_param_subclass.Broad_subclass == subclass_, ])
    hof_param_data_subclass_, conductance_dist_matrix = process_conductance_data(
        hof_param_data_subclass_)
    unique_cell_ids = hof_param_data_subclass_.Cell_id.unique()
    ...
    masked_intra_conductance = np.multiply(
        conductance_dist_matrix, intra_mask_blkdiag).flatten()
    masked_intra_conductance = masked_intra_conductance[masked_intra_conductance != 0]
    masked_inter_conductance = np.multiply(
        conductance_dist_matrix, inter_mask_blkdiag).flatten()
    ...
    _, p_less_ = mannwhitneyu(masked_intra_conductance,
                              masked_inter_conductance, alternative='less')
    ...
    ax[ii // 2, ii % 2] = sns.distplot(intra_dist[subclass_], norm_hist=True,
                                       ax=ax[ii // 2, ii % 2],
                                       hist_kws={'label': 'intra'}, color=intra_cell_intra_class_col)
    ax[ii // 2, ii % 2] = sns.distplot(inter_dist[subclass_], norm_hist=True,
                                       ax=ax[ii // 2, ii % 2],
                                       hist_kws={'label': 'inter'}, color=inter_cell_intra_class_col)
    ax[ii // 2, ii % 2].set_title(subclass_, fontsize=axis_fontsize)
    ...
    heatmap_figname = 'figures/intra_inter_distbn_heatmap_%s.svg' % subclass_
    draw_heatmap(heatmap_figname, conductance_dist_matrix)

handles, labels = ax[ii // 2, ii % 2].get_legend_handles_labels()
...
inh_subclasses = man_utils.inh_subclasses

if plt_inh_dispersion:
    hof_param_data_inh_subclass = hof_param_subclass.loc[hof_param_subclass.Broad_subclass.isin(
        inh_subclasses), ]
    param_df_bcre, cond_inh = process_conductance_data(
        hof_param_data_inh_subclass)
    mask = np.tri(param_df_bcre.shape[0], k=0)
    masked_cond_inh = np.ma.array(cond_inh, mask=mask)
    inh_palette = {
        inh_line_: broad_subclass_color_dict[inh_line_] for inh_line_ in inh_subclasses}
    cre = param_df_bcre.loc[param_df_bcre.Broad_subclass.isin(
        inh_subclasses), 'Broad_subclass'].astype(str)
    ...
    for line in inh_subclasses:
        ...
        intra_class_intra_cell_blkdiag = np.zeros(
            (num_all_cells, num_all_cells))
        intra_class_intra_cell_blkdiag[line_idx:line_idx +
                                       num_line_cells, line_idx:line_idx + num_line_cells] = 1
        intra_class_intra_cell_blkdiag = np.triu(
            intra_class_intra_cell_blkdiag, k=1)
        ...
        masked_intra_conductance = np.multiply(
            avg_dist_mat, intra_class_blkdiag).flatten()
        intra_class_intra_cell_conductance = masked_intra_conductance[
            masked_intra_conductance != 0]

        masked_intra_class_inter_cell_conductance = np.multiply(
            avg_dist_mat, intra_class_intra_cell_blkdiag).flatten()
        intra_class_inter_cell_conductance = masked_intra_class_inter_cell_conductance[
            masked_intra_class_inter_cell_conductance != 0]

        inter_class_conductance = np.multiply(
            avg_dist_mat, inter_class_blkdiag).flatten()
        ...
        sns.distplot(intra_class_intra_cell_conductance,
                     norm_hist=True, ax=ax_d)
        sns.distplot(intra_class_inter_cell_conductance,
                     norm_hist=True, ax=ax_d)
        sns.distplot(inter_class_inter_cell_conductance,
                     norm_hist=True, ax=ax_d)
    ...
    g.fig.savefig('figures/inh_conductance_heatmap.png',
