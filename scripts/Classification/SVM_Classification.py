...
def SVM_classifier(X_df, y_df, feature_fields, target_field, hyp_search='random'):
    ...
    if hyp_search == 'grid':
        ...
        # Grid Search
        param_grid = {
            'svc__C': np.logspace(-2, 3, 100), 'svc__gamma': np.logspace(-4, 0, 10)}
    ...
    df_conf_svm = pd.DataFrame(confusion_matrix_svm, classes,
                               classes)
    df_conf_svm = df_conf_svm.div(df_conf_svm.sum(
        axis=1), axis=0)  # Fraction of the actual no.s
    ...
    test_y_pred, test_y = le.inverse_transform(
        y_pred_test), le.inverse_transform(y_test)
...
mouse_data_df = mouse_data_df.loc[~((mouse_data_df.Broad_Cre_line == 'Pyr') & (
    mouse_data_df.Dendrite_type == 'aspiny')), ]

# Filter inhibitory cells
inh_lines = ["Htr3a-Cre_NO152", "Sst-IRES-Cre", "Pvalb-IRES-Cre"]
mouse_data_df = mouse_data_df.loc[~((mouse_data_df.Cre_line.isin(
    inh_lines)) & (~mouse_data_df.Cell_id.isin(filtered_me_inh_cells)))]

# Filter exc cells
exc_lines = ["Nr5a1-Cre", "Rbp4-Cre_KL100"]
mouse_data_df = mouse_data_df.loc[~((mouse_data_df.Cre_line.isin(
    exc_lines)) & (~mouse_data_df.Cell_id.isin(filtered_me_exc_cells)))]

# %% Feature and target specifications

cre_cluster = mouse_data_df.loc[:, [
    'Cell_id', 'Cre_line']].reset_index(drop=True)
bcre_cluster = mouse_data_df.loc[:, [
    'Cell_id', 'Broad_Cre_line']].reset_index(drop=True)
...
# When the targets are Cre-lines merged from transcriptomic data and not just normal cre
    ...
    cre_cluster['Cre_line'] = cre_cluster.Cre_line.apply(
        lambda x: man_utils.get_cretype_cluster(x, cre_type_cluster))
...
feature_name_list = ['Ephys', 'Morph+Ephys',
                     'Model Parameters', 'Morph+Parameters']
...
for ii, target_ in enumerate(target_field_list):
    for jj, feature_ in enumerate(feature_field_list):
        ...
        feature_fields = [
            feature_field_ for feature_field_ in feature_ if feature_field_ != 'Cell_id']
        ...
        score, conf_df, delta_chance, test_y, test_y_pred = SVM_classifier(
            X_df, y_df, revised_features, target_, hyp_search='grid')

        if target_ == 'Broad_Cre_line':
            conf_df = conf_df.reindex(
                index=bcre_index_order, columns=bcre_index_order)

        elif target_ == 'Cre_line':
            cre_indices = conf_df.index
            sorted_cre_indices = [
                cre_ind_ for cre_ind_ in approved_cre_lines if cre_ind_ in cre_indices]
            conf_df = conf_df.reindex(
                index=sorted_cre_indices, columns=sorted_cre_indices)
            cre_indices = [cre.split('-')[0] for cre in sorted_cre_indices]
            conf_df = pd.DataFrame(
                conf_df.values, index=cre_indices, columns=cre_indices)
        ...
        test_pred_record[ii, jj] = pd.DataFrame(
            {'true': test_y, 'predicted': test_y_pred})
...
for ii in range(fig_dim_x):
    for jj in range(fig_dim_y):
        ...
        if ii == 0:
            ...
            xticktext = [tick.get_text() if ii % 2 == 0 else '' for ii, tick in enumerate(
                xticklabels)]  # For refined Cre-lines show only few x-labels
            feature_title = ''
        ...
        if jj == 0:
            axes[ii, jj].set_ylabel(
                'True Label', labelpad=15, fontsize=axis_fontsize)
            plt.setp(axes[ii, jj].get_yticklabels(), rotation=0, va='center',
                     fontsize=tick_fontsize)
        elif jj == fig_dim_y - 1:
            axes[ii, jj].set_ylabel(
                target_title, labelpad=10, fontsize=axis_fontsize)
        ...
        axes[ii, jj].set_title(
            feature_title + prefix_title + percent_text, fontsize=title_fontsize)
