...
exc_expression_data = exc_expression_data.loc[:, (exc_expression_data != 0).sum(
    axis=0) >= .1 * exc_expression_data.shape[0]]
exc_expression_data = pd.merge(exc_expression_data, annotation_df.loc[:,
                               ...
                               on='sample_id')
h_genes = sorted([ch_g for ch_g in list(
    exc_expression_data) if ch_g.startswith('Hcn')])
...
Rbp4_filtered = exc_expression_data.loc[(~exc_expression_data.primary_cluster_label.
                                        ...
                                        (~exc_expression_data.subclass_label.isin(
                                            ['Sst', 'Vip'])),
                                        'sample_id'].tolist()

filtered_exc_cells = Nr5_filtered + Rbp4_filtered
utility.save_pickle(os.path.join(
    data_path, 'filtered_ttype_exc_cells.pkl'), filtered_exc_cells)
exc_expression_data = exc_expression_data.loc[exc_expression_data.sample_id.isin(
    filtered_exc_cells), ]

# %% Plot gene expression heatmap
# Group the cells according to cre-lines
sorterIndex = dict(zip(exc_lines, range(len(exc_lines))))
exc_expression_data['Cre_rank'] = exc_expression_data['Cre_line']. map(
    sorterIndex)
...
for cre in exc_lines:
    cre_numbers[cre] = len(
        exc_expression_data.loc[exc_expression_data.Cre_line == cre, ])
...
for gene_ in h_genes:
    for comb in combinations(exc_lines, 2):
        cre_x, cre_y = comb
        gene_x = exc_expression_data.loc[exc_expression_data.Cre_line ==
                                         cre_x, gene_].values
        gene_y = exc_expression_data.loc[exc_expression_data.Cre_line ==
                                         cre_y, gene_].values
...
for dtype_ in data_types:
    idx_tuple_ = ('%s.%s' %
                  (dtype_, exc_lines[0]), '%s.%s' % (dtype_, exc_lines[1]))
    idx_list.append(idx_tuple_)

# %% Cummings plot

palette_mod = {comp_type: exc_palette[comp_type.split(
    '.')[-1]] for comp_type in comp_types}
sns.set(font_scale=1.2)
gene_comp_figname = 'figures/diff_gene_expression_exc.svg'
gene_df = dabest.load(exc_expression_melted,
                      idx=idx_list, x="Cre_gene", y='cpm')
...
for label in raw_xticklabels:
    ...
    num_ = txt.split('\n')[-1]
    labels.append('%s\n%s\n%s' %
                  (type_.split('.')[0], type_.split('.')[-1], num_))
