...
hof_param_data = man_utils.read_csv_with_dtype(
    hof_param_data_filename, hof_param_datatypes_filename)
best_param_data = hof_param_data.loc[hof_param_data.hof_index <
                                     hof_num, hof_param_data.columns != 'hof_index']
...
mouse_metadata = mouse_data_best.loc[mouse_data_best.hof_index == 0, [
    'Cell_id', 'Cre_line', 'Layer']]
...
best_param_exc_lines = best_param_with_metadata.loc[best_param_with_metadata.Cre_line.isin(
    exc_lines), ]
...
best_param_exc_lines = best_param_exc_lines.loc[best_param_exc_lines.Cell_id.isin(
    filtered_exc_cells), ]
...
diff_cond_somatic['sig_level'] = diff_cond_somatic['p_val'].apply(
...
best_param_melt_df = pd.melt(best_param_exc_lines, id_vars=['Cell_id', 'Cre_line'],
                             ...
                             value_name='value')
best_param_melt_df = best_param_melt_df.loc[best_param_melt_df.conductance ==
                                            select_params_somatic[0], ]
...
effsize_axes.set_xticklabels(['', 'L5 PT-L4 IT'])
