...
select_subclass_labels <- c("Vip", "Sst", "Pvalb")
select_samples_data <- anno_feather_data[anno_feather_data$subclass_label %in% 
                                      select_subclass_labels, c("sample_id","subclass_label")]
...
subclass_genes <- c('Pvalb','Sst','Vip')
...
all_channel_genes <- c(subclass_genes, ref_genes, select_genes)
...
write.csv(all_gene_expr, file = "inh_expression_subclass.csv", row.names = FALSE)
