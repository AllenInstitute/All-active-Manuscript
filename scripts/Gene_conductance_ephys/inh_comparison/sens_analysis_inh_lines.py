...
mouse_datatype_path = os.path.join(os.getcwd(),os.pardir,'Mouse_class_datatype.csv')
...
sa_df_inh_lines['param_name'] = sa_df_inh_lines['param_name'].apply(lambda x:rename_channel(x))
channel_sorted = sorted(sa_df_inh_lines.param_name.unique().tolist(),reverse=True)
...
tick_fontsize=10
axis_fontsize=10
g= sns.FacetGrid(data=sa_df_inh_lines,col='feature',col_order=select_features)
