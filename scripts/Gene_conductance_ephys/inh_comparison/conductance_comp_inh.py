...
filtered_inh_expression_path = os.path.join(data_path,'inh_expression_filtered.csv')
#%% Read the data
...
hof_param_data_cre =  pd.merge(hof_param_data,cre_cluster,how='left',on='Cell_id')
...
hof_param_data_cre = pd.merge(hof_param_data_cre,me_cluster_data,on='Cell_id')
hof_param_data_inh_lines = hof_param_data_cre.loc[hof_param_data_cre.Cre_line.isin(inh_lines),]
...
utility.save_pickle(os.path.join(data_path,'filtered_me_inh_cells.pkl'),filtered_me_inh_cells)
...
conductance_params = [dens_ for dens_ in list(hof_param_data_inh_lines) if dens_ not in ['Cell_id','hof_index','Broad_Cre_line'] and dens_.startswith('gbar')]

somatic_params = [cond_param for cond_param in conductance_params if 'somatic' in cond_param]
...
for param_ in somatic_params:
    for comb in combinations(inh_lines, 2):
        cre_x,cre_y = comb
        param_x = hof_param_data_inh_lines.loc[hof_param_data_inh_lines.Cre_line == cre_x,param_].values
        param_y = hof_param_data_inh_lines.loc[hof_param_data_inh_lines.Cre_line == cre_y,param_].values
...
diff_channel_cond_df['sig_level'] = diff_channel_cond_df['p_val'].apply(lambda x:man_utils.pval_to_sig(x))
...
for gene_ in gene_types:
    for comb in combinations(inh_lines, 2):
        cre_x,cre_y = comb
        gene_x = inh_expression_data.loc[inh_expression_data.Cre_line == cre_x,gene_].values
        gene_y = inh_expression_data.loc[inh_expression_data.Cre_line == cre_y,gene_].values
...
diff_gene_expression_df['sig_level'] = diff_gene_expression_df['p_val'].apply(lambda x:man_utils.pval_to_sig(x))
...
for channel_,genes in  channel_correlate_dict.items():
    ...
    ax1.set_ylabel(r'$\mathrm{\bar{g}\:(S cm^{-2})}$',fontsize=axis_fontsize+2)
    ax1.set_xlabel('channel',fontsize=axis_fontsize)
    box = ax1.get_position()
    ax1.set_position([box.x0, box.y0, box.width, box.height]) # resize position
    ax1.legend(loc='lower center', bbox_to_anchor=(1,- 0.2), ncol=3,fontsize=
               axis_fontsize,frameon=False)
    ax1.set_yscale("log")
    hue_var = 'Cre_line'
    sns.despine(ax=ax1)
    ...
    fig.savefig('figures/diff_expression_%s_inh.pdf'%channel_,bbox_inches='tight')
